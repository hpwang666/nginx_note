...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    */
    ...
    这里之后会详细讲解的哦
    */
    (void) ngx_process_events(cycle, timer, flags);//事件都会post到队列，释放锁了再处理
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "timer delta: %M", delta);
	/*
    ngx_posted_accept_events 是一个事件队列，暂存epoll从监听套接口wait到的accept事件。
    ...
    */
	...
	/*
    delta是之前统计的耗时，存在毫秒级的耗时，就对所有时间的timer进行检查，如果为0，就不需要检查，很短时间被触发
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    cycle->free_connections = next;//在这里一次性将所有conn初始化，以备调用
