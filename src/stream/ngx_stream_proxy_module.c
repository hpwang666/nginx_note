...
ngx_stream_proxy_connect(ngx_stream_session_t *s)
{
    ...
    if (rc != NGX_AGAIN) {
        ngx_stream_proxy_init_upstream(s);//直接可以初始化了
        return;
    }

    pc->read->handler = ngx_stream_proxy_connect_handler;
    pc->write->handler = ngx_stream_proxy_connect_handler;//write超时函数

    ngx_add_timer(pc->write, pscf->connect_timeout);//加入定时器去判断
}
...
ngx_stream_proxy_connect_handler(ngx_event_t *ev)
{
    ...
    if (ngx_stream_proxy_test_connect(c) != NGX_OK) {//getsocket err
        ngx_stream_proxy_next_upstream(s);
        return;
    }

    ngx_stream_proxy_init_upstream(s);//可以初始化连接了。
}
